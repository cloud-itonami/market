(ns cloud-itonami.market.app
  "cloud-itonami/market appview (etzhayyim-wasm-market-mk7r3x9p) — reagent +
  ...
  Two kinds of change were made to that constant, and both are spelled
  out here rather than silently applied:

  - `:app/relative-path` now names this file, not the deleted Svelte one.
  - `:app/route-count`, `:app/routes`, and `:app/vars` are corrected, not
    copied. The Svelte scaffold's `app` constant held `routeCount: 0`,
    `routes: []`, and `vars: []`, but
    `appview/etzhayyim-wasm-market-mk7r3x9p/wrangler.jsonc` actually
    declares two routes (`mk7r3x9p.etzhayyim.com/*`,
    `market.etzhayyim.com/*`) and ten `vars` keys — the generated status
    page was stale on both points. `:app/vars` below is that real key set
    (sorted), not the Svelte scaffold's empty `vars: []`.

  `:app/xrpc?` stays `true`, and unlike a sibling migration
  (cloud-itonami-isic-6110/worker) this is not stale: this Worker's
  `src/app.ts` implements its own `/xrpc/*` handler (dispatching to
  `DISPATCHER_URL` for a fixed `MARKET_NSIDS` set) and falls through to
  `env.ASSETS.fetch` for everything else, so `wrangler.jsonc` keeps
  `main: ./src/app.ts` and XRPC really is wired into the deploy. That is
  a *different* XRPC path from the one the old SvelteKit server route
  implemented (`+server.ts` proxied to `AGENTGATEWAY_MCP_ROUTER_URL` as
  an MCP `tools/call`) — that handler was moved byte-for-byte (plus a
  header) to `src/xrpc-mcp-router-proxy.ts` and is preserved but not
  wired to anything; whether to revive it is an open product decision,
  not one this frontend migration made.
  ...
  components or ext-rules change) with:
    ...
    (spit \"public/index.html\"
          (page/->page {:title \"etzhayyim-wasm-market-mk7r3x9p\"
                         :lang \"ja\"
                         :description \"market (etzhayyim-wasm-market-mk7r3x9p) — Cloudflare worker appview (reagent + re-frame + jp-go-dds).\"
                         :css (slurp (io/resource \"jp_go_dds/dds.css\"))}
                        [:div {:id \"app\"} \"etzhayyim-wasm-market-mk7r3x9p loading…\"]
...
;; with route-count/routes/vars corrected to
;; appview/etzhayyim-wasm-market-mk7r3x9p/wrangler.jsonc's real values
;; (see namespace docstring).

(def default-db
  {:app/title "Market Mk7r3x9p"
   :app/project "etzhayyim-project-market"
   :app/name "etzhayyim-wasm-market-mk7r3x9p"
   :app/kind "appview"
   :app/route-count 2
   :app/routes ["mk7r3x9p.etzhayyim.com/*" "market.etzhayyim.com/*"]
   :app/vars ["AGENTGATEWAY_MCP_ROUTER_URL" "APP_ACTOR_HANDLE" "APP_CAPABILITIES"
              "APP_DESCRIPTION" "APP_DISPLAY_NAME" "APP_EMBED_URL" "APP_FRAMEWORK"
              "APP_NANOID" "APP_PERFORMER_TYPE" "APP_UI_TYPE"]
   :app/xrpc? true
   :app/relative-path "appview/etzhayyim-wasm-market-mk7r3x9p/cljs/src/cloud_itonami/market/app.cljs"})
