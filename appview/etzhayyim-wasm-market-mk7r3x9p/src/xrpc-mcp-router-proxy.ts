...
//
// Moved verbatim from
// `svelte/src/routes/xrpc/[...path]/+server.ts` (only this header comment
// was added; the handler logic below is byte-for-byte unchanged). That was
// the SvelteKit server-route file that proxied XRPC calls to
// AGENTGATEWAY_MCP_ROUTER_URL as an MCP `tools/call` before this migration.
//
// It still imports `json`/`RequestEvent` from '@sveltejs/kit' and
// `RequestHandler` from './$types' (SvelteKit's codegen'd route types),
// neither of which resolves now that the SvelteKit toolchain
// (`appview/etzhayyim-wasm-market-mk7r3x9p/svelte/`) has been removed as
// part of the Svelte-to-ClojureScript FRONTEND migration (see the repo
// README and this app's wrangler.jsonc header comment). `src/app.ts`
// (this Worker's `main`) already implements its own, separate `/xrpc/*`
// handler that dispatches to DISPATCHER_URL for a fixed NSID set — it does
// not call into this file, so this proxy is not currently wired to any
// deploy target.
//
// Whether/how to revive this XRPC proxy (e.g. rewritten as a plain
// Cloudflare Worker fetch handler with no SvelteKit dependency, or folded
// into src/app.ts's own /xrpc/ dispatch) is an open product decision, not
